...
[consume], `mark-consumed` [produce] — операционные идентификаторы перенесены as-is.

Типы полей контракта восстановлены из кода mailtg и data-config (детали в сайдкаре + migration-report.md §6).
Остаются ОТКРЫТЫМИ (human-gate) 3 поля, не материализованные в коде — владельцу решить, нужны ли они в контракте:
- `SendMailSuccess.accepted`, `MarkConsumedSuccess.consumed` — код не строит объект результата (успех = отсутствие исключения);
- `SendMailInput.notices[]` — в коде это отдельные вызовы `send_notice()`, а не коллекция в запросе send-mail.
...
(-> patterns/sidecar-formats.md). Типы части полей не зафиксированы в исходной прозе контракта —
см. `#`-пометки `MIGRATE-TODO` в сайдкаре и раздел «Машинная спецификация» выше.
