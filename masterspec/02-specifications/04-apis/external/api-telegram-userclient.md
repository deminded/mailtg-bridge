...
не выдумывай»).
<!-- MIGRATE-TODO: машинная проекция — стандарт под транспорт (MTProto RPC через Telethon) не
     определён. К владельцу: (а) осознанно принять "компаньон без сайдкара" как решение по этому API,
     или (б) указать конкретный стандарт/формат, если он всё же применим. -->
