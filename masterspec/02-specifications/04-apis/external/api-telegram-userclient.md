...
- **Business-reject codes:** N/A — контракт MTProto-клиента не содержит отдельных синхронных
  бизнес-кодов отказа; полная таксономия отказов исчерпана транспортной таблицей ниже
  (FloodWait | transient | session-invalid | peer-not-found | media-unavailable | bad-code |
  bad-password).
...
- непрерывность выборки: fetch-messages-since возвращает id строго > cursor в хронологическом порядке;
  непрерывность (отсутствие дыр от удалённых сообщений) НЕ гарантируется и НЕ требуется — курсор-high-watermark
  (-> alg-batch-per-dialog-cycle, -> alg-dedup-idempotency) делает пропуски безвредными, отдельного
  обнаружения дыр не нужно.
...
session-invalid относится к ЛЮБОЙ операции: отказ post-message-as-user из-за сессии/авторизации при
рабочем fetch трактуется так же — консервативно как session-invalid → -> scn-session-invalid-alert
(единая бинарная трактовка «действительна/недействительна», без состояния «частично недействительна»).
...
### Машинный sidecar
Не создаётся. Транспорт — MTProto RPC поверх пользовательской сессии (Telethon) — не имеет готового
машинного стандарта, под который эту usage-контракт-абстракцию можно спроецировать без выдумывания
путей/методов: сырая TL-схема MTProto описывает протокол Telegram-клиента на уровне, которого этот
артефакт не касается (операции здесь — логическая абстракция моста: `list-tracked-dialogs`,
`fetch-messages-since` и т.д. — не имена реальных TL-методов), а OpenAPI/AsyncAPI/protobuf/WSDL/SDL
предполагают транспорт (HTTP/очереди/gRPC/GraphQL), которого здесь тоже нет. Синтезировать
OpenAPI/AsyncAPI с фиктивными путями/операциями — запрещено (-> patterns/sidecar-formats.md, «нотацию
не выдумывай»).
**Решение владельца (14.07): компаньон без машинного сайдкара — осознанная форма этого API.**
MTProto RPC не имеет готового стандарта машинной проекции; синтетический OpenAPI/AsyncAPI был бы
выдумыванием путей/операций. Артефакт ведётся как `.md`-компаньон (без `sidecar_format`/`sidecar` во
фронтматтере) — это записанное решение, не пробел; migrate/verify его уважают (не переопределять без
нового решения владельца).

## Реализация OE-DELIVERY (обратная ссылка)
Этот API — точка подтверждения (server ack) для доставки следующей функции; контракт «доставлено» —
у владеющего сценария, здесь фиксируется только сам факт участка доставки:
- -> fn-email-reply-to-tg/OE-DELIVERY — Telegram принял `send_message` (публикация ответа от лица пользователя).
