...
конфигурируем; контракт ключа -> data-config); команда обрабатывается в пределах одного такого
интервала независимо от нагрузки на другие функции. Источник: код `config.py`
(`SEND_INTERVAL_SECONDS`), `orchestrator.py` (`run_mailbox_cycle`).
