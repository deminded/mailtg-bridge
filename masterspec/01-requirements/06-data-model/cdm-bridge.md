...
- **Состояние сессии** — здоровье сессии Telegram: действительна либо недействительна/отозвана;
  отдельное от переключателя доставки блокирующее состояние (при недействительной сессии опрос
  прекращается до повторной инициализации).
...
порог размера вложений; лимит размера письма (провайдера); политика хранения (retention) журнала
связки; периодичность сбора; периодичность отправки; опциональный секрет-токен команды.

**Состояние моста:** признак «доставка включена/выключена».

**Состояние сессии:** признак «сессия действительна / недействительна (отозвана)».
...
- Если продвижение курсора диалога или запись связки не удались (в т.ч. при успешно отправленном
  письме) — курсор НЕ продвигается: допускается повторная доставка на следующем такте (at-least-once,
  дубль приемлем), потеря сообщения — нет.
- Ответ-письмо потребляется единожды: повторный опрос ящика или перезапуск не публикует уже
  обработанный ответ в Telegram повторно (идемпотентность приёма).
- Состояние сессии отдельно от переключателя доставки: при недействительной/отозванной сессии опрос
  Telegram прекращается и пользователю (U) уходит письмо-уведомление; повторного зацикливания на той
  же ошибке нет (блокирующее состояние до повторной инициализации).
- Журнал связки не растёт неограниченно: записи хранятся согласно политике хранения (retention) из
  конфигурации, устаревшие вычищаются.
...
| Сообщение | batched | письмо отправлено, но запись связки/продвижение курсора не удались | → gated-passed (курсор НЕ продвинут; повторная доставка на след. такте, at-least-once — дубль приемлем, потеря нет) | при записи |
...
| Сообщение | replied | НОВЫЙ (иной) ответ письмом на тот же диалог | → replied (допускается доп. публикация нового ответа; дедупа по содержанию нет) | при записи |
| Сообщение | delivered / replied | повторный опрос / перезапуск с ТЕМ ЖЕ ответом-письмом | → без изменений (ответ потреблён единожды, повторно в TG не публикуется) | при чтении |
...
### Состояние сессии (действительна ↔ недействительна/отозвана)
Отдельно от переключателя доставки; недействительность — блокирующее (терминальное до реинициализации) состояние.

| Сущность | Состояние | Событие | → Результат (или ЗАПРЕЩЕНО, почему) | Момент перехода |
|---|---|---|---|---|
| Состояние сессии | действительна | опрос/публикация проходят | → действительна (работа идёт) | по расписанию |
| Состояние сессии | действительна | Telegram отклонил сессию (отозвана/недействительна) | → недействительна (опрос остановлен, письмо-уведомление пользователю U) | при записи |
| Состояние сессии | недействительна | наступил такт сбора/отправки | → недействительна (опрос НЕ идёт; повторного уведомления/зацикливания нет — блокирующее состояние) | по расписанию |
| Состояние сессии | недействительна | выполнена повторная интерактивная инициализация (-> fn-first-run-setup) | → действительна (сервис возобновляет работу) | при записи |
